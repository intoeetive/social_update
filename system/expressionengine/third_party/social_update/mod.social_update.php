...
class Social_update {
    ...
    function post_delayed($entry_id=false)
    {
		...
		foreach ($posts->result_array() as $post_data)
		{
			if (in_array($post_data['status'], $module_settings[$post_data['site_id']]['trigger_statuses']) && $post_data['entry_date']<=$this->EE->localize->now)
			{
		        ...
		        if (strlen(urlencode($data." ".$url)) > $this->maxlen[$post_data['service']])
		        {
		            $data = $this->_char_limit($data, ($this->maxlen[$post_data['service']]-strlen(urlencode($url))-1));
		        }                  
    ...
    function _char_limit($str, $length, $minword = 3)
    {
        ...
        foreach ($words_a as $i=>$word)
        {
            ...
            $check = (isset($words_a[$i+1]))?($sub.' '.$words_a[$i+1]):$sub;
        ...
        if (($len < strlen($str)) && ($length-$len < 3))
        //special case if shortened string is less that 3 char shorter
        //remove one more word so we could fit '...' in
        {
